...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'castle': //Glitchy
				var bg:BGSprite = new BGSprite('castlestage', -600, -200, 0.9, 0.8, 'char-smg4');
				add(bg);

				if (!ClientPrefs.lowQuality) {
					var glitchybois:BGSprite = new BGSprite('glitchygang', -90, 30, 0.9, 0.8, 'char-smg4', ['glitchy gang'], true);
					glitchybois.updateHitbox();
					add(glitchybois);
					
					var smg3:BGSprite = new BGSprite('wtfamiseeing', 365, -56, 0.9, 0.8, 'char-smg4', ['wtfamiseeing'], true);
					smg3.setGraphicSize(Std.int(smg3.width * 0.65));
					smg3.updateHitbox();
					add(smg3);
				}
			case 'stage': //Tutorial
		...
		timeTxt.setFormat(Paths.font("smg.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("smg.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("smg.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		gameText = new FlxText(0, FlxG.height - 42, 500, songName + "", 32);
		gameText.text = SONG.song + " - [" + CoolUtil.difficultyStuff[storyDifficulty][0] + "]";
		gameText.setFormat(Paths.font("smg.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		gameText.scrollFactor.set();
		gameText.borderSize = 1.25;
		add(gameText);
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char);
		#end
		super.create();
	}
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char, true, songLength);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char, true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char);
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char);
		}
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{ //Note: Used For Event's (I Think) -Generalisk
		}
		...
		if (FlxG.keys.justPressed.SEVEN && !endingSong && !inCutscene)
		{
			...
			#if desktop
			DiscordClient.changePresence("Chart Editor", null, "icon-" + iconP2.char, true);
			#end
		}
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon-" + iconP2.char);
				#end
				isDead = true;
				return true;
			}
	...
	public function endSong():Void
	{
		...
		if (achievementObj == null) //Shadow Man WTF! Why You Use != Instead Of ==. That Just Softlock's The Achievement System -Generalisk
		{
			var allowReturn:Bool = false;
			//SMG4 Song's
			var achieve:String = checkForAchievement(['char_smg4_beat']);
			if(achieve != null)
			{
				allowReturn = true;
			...
			var achieve:String = checkForAchievement(['char_smg4_beat_hard']);
			if(achieve != null)
			{
				allowReturn = true;
				startAchievement(achieve);
				
				var achieve:String = checkForAchievement(['char_smg4_aimbot']);
				if(achieve != null)
				{ //No Point On Putting "allowReturn = true;" Here -Generalisk
					startAchievement(achieve);
				}
			}

			if (allowReturn) //Prevent's Softlock -Generalisk
			{
				return;
			}
		}
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore && !usedPractice && !cpuControlled)
			{
				#if !switch
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{ //Note: Used For Custom Stage Animation's -Generalisk
		}
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				{
					case 'char_smg4_beat':
						if (Paths.formatToSongPath(SONG.song).toLowerCase() == "glitchy")
						{
							unlock = true;
						}
					case 'char_smg4_beat_hard':
						if (CoolUtil.difficultyString() == 'HARD' && Paths.formatToSongPath(SONG.song).toLowerCase() == "glitchy")
						{
							unlock = true;
						}
					case 'char_smg4_aimbot':
						if (CoolUtil.difficultyString() == 'HARD' && Paths.formatToSongPath(SONG.song).toLowerCase() == "glitchy" && songMisses < 1)
						{
							unlock = true;
						}
				}
